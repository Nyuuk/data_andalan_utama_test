...
export default function Products() {
    ...
    useEffect(() => {
        API.getTransactions(pagination.perPage, 1, valueSearch)
        ...
        <Layout {...addtionalProps}>
            <>
                <TableT
                    ...
                    clickLast={() => {
                        API.getTransactions(pagination.perPage, data.last_page, valueSearch)
                            .then((result) => {
                                if (result !== false) {
                                    setData(result);
                                    return;
                        ...
                        onSubmit={() => {
                            API.deleteProduct(modalDelete.data).then(
                                (result) => {
                                    ...
                                    API.getTransactions(20, 1,valueSearch).then((result) => {
                                        if (result !== false) setData(result);
                                    });
                                    showAlert(
                            ...
                            if (modalEdit.type === "edit") {
                                API.editProduct(modalEdit.data).then(
                                    (result) => {
                                        ...
                                        API.getTransactions(20, 1, valueSearch).then(
                                            (result) => {
                                                if (result !== false)
                                                    setData(result);
                                            }
